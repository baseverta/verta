...
**Versão:** 3.0  
**Status:** calibrada após análise de unidade econômica e mercado  
**Data:** agosto de 2026
...
A precificação tem duas rotas:

1. **Oferta de entrada padronizada (Verta Start)** para negócios com faturamento mensal de até R$ 15.000: facilita a decisão, reduz esforço comercial e gera casos reais.
...
A Rota 1 foi reajustada na v3 para valores mais sustentáveis (setup R$ 2.800 + R$ 690/mês), mantendo competitividade como porta de entrada. A Rota 2 recebeu correção de hora-base, buffer e mensalidade mínima para proteger margem sem perder competitividade.
...
- Todos os projetos devem ter apontamento de horas, incidentes, suporte e custo de ferramentas. Os valores desta versão serão recalibrados com dados reais das primeiras entregas.
- Nunca prometer resultado financeiro sem dados fornecidos pelo cliente e um método de aferição acordado.

## Rota 1: Oferta de Entrada (Verta Start)

### Público

Negócios com faturamento mensal entre **R$ 3.500 e R$ 15.000**. A Verta não recusa esse público, desde que o escopo seja repetível e a operação tenha condição mínima de usar WhatsApp, agenda ou CRM/planilha.

### Objetivo

- Gerar caixa com unidade econômica minimamente saudável.
- Criar 3 cases instrumentados de qualidade.
...
| Setup | R$ 2.800, preço fixo |
| Operação mensal | R$ 690/mês, preço fixo |
...
### Critério de saída da fase de calibração

A Rota 1 no valor atual deve ser revista assim que forem atingidos:

1. 3 cases instrumentados com métricas reais (tempo de 1ª resposta, % resolvido sem humano, leads recuperados fora do horário).
2. Apontamento de horas reais de implantação + suporte dos primeiros 60–90 dias.
3. Demanda recorrente suficiente para não depender do preço como isca principal.

Não existe número mágico de clientes. A decisão de manter ou subir é orientada por dados de unidade econômica, não por volume arbitrário.
...
```text
setup dos módulos = soma(horas dos módulos × R$ 85)
setup recomendado = setup dos módulos × 1,20
```

O fator de 1,20 é um buffer de 20% para ajustes previsíveis de implantação, atrasos de acesso e pequenas variações de escopo. Não há cobrança fixa de “base operacional” nesta versão. Quando o diagnóstico revelar demanda alta, dados ruins, urgência ou integrações não padronizadas, a Verta deve precificar isso como Fundação, módulo adicional ou escopo personalizado — não esconder o esforço no buffer.

**Travas atuais:** mínimo de setup de R$ 1.800 e teto de referência de R$ 12.000. Acima do teto, dividir a implantação em fases ou redesenhar o escopo.

### Serviços modulares

| # | Serviço | Escopo resumido | Horas de referência | Setup-base | Mensalidade |
|---|---|---|---:|---:|---:|
| 1 | Atendimento 24/7 com IA | WhatsApp oficial, respostas inteligentes, triagem e handoff humano | 18h | R$ 1.530 | R$ 360/mês |
| 2 | Agendamento automático | Agenda, confirmação, reagendamento e lembretes | 12h | R$ 1.020 | R$ 225/mês |
| 3 | Registro e movimentação de CRM | Criação de lead, preenchimento de campos e movimentação de cards | 10h | R$ 850 | R$ 180/mês |
| 4 | Agente SDR (qualificação) | Critérios de fit, qualificação e entrega ao comercial | 10h | R$ 850 | R$ 180/mês |
| 5 | Agente de renovação | Retomada de clientes, renovação e oportunidades de upsell | 10h | R$ 850 | R$ 180/mês |
| 6 | Agente de suporte | Triagem pós-venda, resolução inicial e abertura de tickets | 10h | R$ 850 | R$ 180/mês |
| 7 | One-Page / Hot-Page | Página Webflow para captação com formulário integrado ao CRM | 14h | R$ 1.190 | R$ 135/mês |

**Exemplo:** Atendimento + Agendamento = R$ 2.550 de setup-base. Com buffer de 20%, o setup recomendado é R$ 3.060. A mensalidade é R$ 585/mês (aplica-se o piso de R$ 690 quando a soma dos módulos for inferior).
...
**Mensalidade mínima na proposta modular:** R$ 690/mês. A calculadora soma as mensalidades dos módulos e aplica esse piso quando necessário.
...
6. Se R$ 2.800 + R$ 690 do Verta Start está gerando caixa, aprendizado e case suficientes.
...
| Item | v2 | v3 |
|---|---|---|
| Rota 1 – Setup | R$ 2.000 | R$ 2.800 |
| Rota 1 – Mensalidade | R$ 450 | R$ 690 |
| Rota 2 – Hora-base | R$ 70 | R$ 85 |
| Rota 2 – Buffer | 12,5% | 20% |
| Rota 2 – Mensalidade mínima | R$ 500 | R$ 690 |
| Alerta de churn | 3% do faturamento | Mantido |
| Critério de saída da Rota 1 promocional | Implícito | Explícito (3 cases + horas reais + demanda) |
